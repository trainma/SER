'''
Author: Xia hanzhong
Date: 2022-06-05 20:01:54
LastEditors: a1034 a1034084632@outlook.com
LastEditTime: 2022-06-14 20:22:53
FilePath: /Speech-Emotion-Recognition/TestMetric.py
Description: 
...
def CreateDataloader(train_x, train_y, val_x, val_y, batch_size, shuffle=True):
    ...
    val_x_tensor = torch.from_numpy(val_x).type(torch.FloatTensor).to(
        Device)  # (B, N, F, T)
...
if __name__ == '__main__':
    ...
    train_dataloader, val_dataloader = CreateDataloader(x_train,
                                                        y_train,
                                                        x_test,
                                                        y_test,
                                                        batch_size=64)
    ...
    for X, y in val_dataloader:
        output = model(X)
        pred = output
        correct += (pred.argmax(1) == y.argmax(1)).type(
