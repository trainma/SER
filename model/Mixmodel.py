...
class MixModel(nn.Module):
    def __init__(self, d_model, batch_size, gru_num_layers, gru_hidden_size, enc_num_layers, dropout, num_class,
                 bidirectional=True):
        super(MixModel, self).__init__()
        # self.temporal = TemporalBlock(n_inputs=1, n_outputs=1, kernel_size=3, stride=1, dilation=2, padding=1, dropout=dropout)
        self.Residual = Residual(hidden_size=2 * gru_hidden_size, gru_num_layers=gru_num_layers, dropout=dropout,
                                 bidirectional=True)
        self.transformer = TransAm_audio(feature_size=d_model, batch_size=batch_size, feature_dim=1576,
        ...
        self.bigru = nn.GRU(input_size=1576, hidden_size=gru_hidden_size, num_layers=gru_num_layers, dropout=dropout,
...
if __name__ == '__main__':
    ...
    Model = MixModel(d_model=2048, batch_size=64, gru_num_layers=3, gru_hidden_size=128, enc_num_layers=1, dropout=0.3,
                     num_class=7)
    out = Model(test)
    print(out)
