...
def get_feature_opensmile(config, filepath: str) -> list:
    ...
    # Opensmile 配置文件路径
    opensmile_config_path = os.path.join(config.opensmile_path, config.opensmile_config + '.conf')

    # Opensmile 命令
    # 'home/Tabe/Downloads/opensmile-master/progsrc/smilextract/SMILExtract -C '
    cmd = 'cd ' + '/' + config.opensmile_path + ' && /home/Tabe/Downloads/opensmile-master/build/progsrc/smilextract/SMILExtract -C ' + '/' + opensmile_config_path + ' -I ' + filepath + ' -O ' + single_feat_path + ' -appendarff 0'
