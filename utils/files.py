    ...
    for root, dirs, files in os.walk(file_path):
        for item in files:
            if item.endswith('.wav'):
                ...
                new_path = os.path.join(root, item_name + '-' + people_name + '.wav')  # 新音频路径
                try:
...
#                 item_name = item[:-4]  # 音频原名（去掉.wav）
#                 old_path = os.path.join(root, item)
#                 new_path = os.path.join(root, item_name + '-' + dataset_name + '-' + people_name + '.wav')  # 新音频路径
#                 try:
#                     os.rename(old_path, new_path)
#                     print('converting ', old_path, ' to ', new_path)
#                 except:
...
def move2(file_path: str) -> None:
    """把音频按情感分类，放在不同文件夹下"""
    for root, dirs, files in os.walk(file_path):
        for item in files:
            if item.endswith('.wav'):
                emotion_name = item[:-4].split('-')[2]
                old_path = os.path.join(root, item)
                folder_path = os.path.join(file_path, emotion_name)
